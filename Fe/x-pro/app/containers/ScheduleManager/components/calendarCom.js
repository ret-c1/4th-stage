...
// } from '../api';
import { calendarComdatajson } from './data';
...
const CalendarCom = (props) => {
    const { state, source, formValues } = props;
    ...
    const END_MONTH = moment(moment().endOf('month').format('YYYY MM DD 23:59:59')).valueOf();
    ...
    useEffect(() => {
        setScheduleDataList(calendarComdatajson.data || []);
    }, []);

    const changeTimeParams = (dateType, chooseTime) => {
        const monthParam = {
            startDate: moment(moment().format('YYYY MM 01 00:00:00')).valueOf(),
            endDate: moment(moment().endOf('month').format('YYYY MM DD 23:59:59')).valueOf(),
        };
        const weekParam = {
            startDate: moment(
                moment()
