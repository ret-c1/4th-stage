...
import { Card, Form, Button, Row, Col, Input, Tree, Menu, Modal } from 'antd';
...
const SystemRoles = () => {
    ...
    console.log(setRoleTreeData, setRole, inputRole);
    ...
    const queryRoleTree = () => {
        ...
        //                             setExpandedKeys(item.permissionIds);
        //                             setChooseRole(item);
        //                         }
        //                     });
        //                 } else {
        //                     setCheckedKeys(res1.data.records[0].permissionIds);
        //                     setExpandedKeys(res1.data.records[0].permissionIds);
    ...
    // 编辑角色权限
    const onFinishMenu = () => {
        const newKeys = checkedKey;
        loop(roleTreeData).forEach((item) => {
            if (!checkedKey.includes(item.key) && item.children && item.children.length > 0) {
                item.children.forEach((item1) => {
                    if (checkedKey.includes(item1.key)) {
                        newKeys.push(item.key);
                    } else if (item1.children && item1.children.length > 0) {
                        item1.children.forEach((item2) => {
                            if (checkedKey.includes(item2.key)) {
                                newKeys.concat([item1.key, item.key]);
                            }
                        });
                    }
                });
