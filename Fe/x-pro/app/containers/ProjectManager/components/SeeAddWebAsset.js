...
import { getWebAssetDetail } from '../page/api';

const SeeAddWebAsset = (props) => {
    const { id } = props;
    const [form] = Form.useForm();
    const [webData, setWebData] = useState({});
    useEffect(() => {
        getWebAssetDetail({ id }).then((res) => {
            if (res.code === 200) {
                setWebData(res.data);
            } else {
                console.log('获取数据失败');
                ...
                <Row>
                    <Col span={12}>
                        <Form.Item label="应用标签：" name="username">
                            {webData.username === '' ? '暂无' : webData.username}
                        </Form.Item>
                    </Col>
                    <Col span={12}>
...
export default SeeAddWebAsset;
SeeAddWebAsset.propTypes = {
    id: PropTypes.number,
};
