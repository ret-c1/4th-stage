...
import { getUpdateSecurityRecord, getCheckSecurityRecord } from '../api';

const { Option } = Select;
const { TextArea } = Input;

const EditRecordChange = (props) => {
    const [form] = Form.useForm();
    const { id } = searchParams();
    const { handleCancel, idRecord } = props;
    ...
    useEffect(() => {
        getCheckSecurityRecord(idRecord).then((res) => {
            if (res.code === 200) {
...
export default EditRecordChange;
EditRecordChange.propTypes = {
