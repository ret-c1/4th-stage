...
const AssessList = () => {
    ...
    const tableParam = useTableParam(getExpertList);
        ...
        render: (text, record) => {
            if (record.status === 1) {
                return (
                    <Button
                        ...
                        onClick={() => {
                            history.push(
                                `/intelligence/assessInfo?type=assess&id=${record.id}&role=infoExpert`,
                            );
                        }}
                    >
                        审核
            ...
            return (
                <Button
                    ...
                    onClick={() => {
                        history.push(
                            `/intelligence/assessFinish?type=check&id=${record.id}&role=infoExpert`,
                        );
                    }}
                >
                    查看
    ...
    return (
        <ScItWrapper>
            <Row justify="space-between" style={{ marginBottom: '12px' }}>
                <ScItTitle>情报审核-情报专家</ScItTitle>
                <div>
                    {/* <Button style={{ marginRight: '4px' }}>情报导入 </Button> */}
                    <Button
                        type="primary"
                        onClick={() => {
                            history.push('/intelligence/list/create?enterBy=infoExpertList');
                        }}
                    >
                        情报上报
                    </Button>
