...
const WarningAddEdit = (props) => {
    ...
    const [destIpList, setDestIpList] = useState([]);
    ...
    // 此时告警名称，来源IP，告警次数来源自告警事件对象; 目的IP,目的系统源自于所选隐患资产，但是可能有只选隐患资产没选告警事件的情况，或者二者都没选
    useEffect(() => {
        ...
        if (assetRecordSource && assetRecordSource.length > 0) {
            assetRecordSource.forEach((item) => {
                ...
                destIps.push(item.assetIp);
                businessSystems.push(item.businessSystem);
            });
            ...
            setAssetIds(assetId);
            setBusinessSystemList([...new Set(businessSystems.join(','))]);
            setDestIpList(destIps);
            setNameList(names);
        }
        form.setFieldsValue({
            ...initialValues,
            name: names[0],
            destSystem: [...new Set(businessSystems)].join(','),
        });
        return () => {
            form.resetFields();
        };
    }, [assetRecordSource, warningRecordSource]);
    ...
    const onFinish = (values) => {
        ...
        if (destIpList.length > 0) {
            if (values.destIps) {
                params.destIps = destIpList.concat(values.destIps).join(',');
            } else {
                params.destIps = destIpList.join(',');
            }
        }
    ...
    return (
        <Form
            ...
            form={form}
            onFinish={onFinish}
            // initialValues={initialValues} // initialValues只对初始渲染和数据重置时有效
            labelCol={{ span: 3 }}
            wrapperCol={{ span: 20 }}
        >
            ...
            <Row>
                ...
                <Col span={6}>
                    <Form.Item
                        ...
                        rules={[
                            {
                                required: destIpList.length === 0 && true,
                                message: '请输入目的IP!',
                            },
                        ]}
                    >
