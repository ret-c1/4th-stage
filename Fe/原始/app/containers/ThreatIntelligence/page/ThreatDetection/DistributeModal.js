...
import { Button, Row, Col, Form, Select, DatePicker, Tree } from 'antd';
...
import SearchInput from '@components/PubSearchInput';
...
const convertAssetData = (data) => {
    ...
    data.forEach((item) => {
        const obj = {
            ...
            key: item.type,
            icon: <FolderOutlined />,
            children: item.assets.map((ele, idx) => ({
                title: ele,
                key: `${ele}-${idx}`,
...
const DistributeModal = (props) => {
    ...
    const { remoteData } = remoteSelectParam;
                    ...
                    {modalFormConfig.map((item) => {
                        if (item.label === '执行者') {
                            return (
                                ...
                                >
                                    <SearchInput name={item.name} form={form} />
                                </Form.Item>
                            );
                        }
                        if (item.label === '执行时间') {
