    ...
    const onOk = debounce(() => {
        ...
        if (actionType === 0) {
            expertRefuse(noPassParams).then((res) => {
                if (res.code === 200) {
                    history.push(
                        `/intelligence/approved-${type}th/evendetail?stage=detail&sourceType=0&id=${id}`,
                    );
        ...
        form.validateFields().then((values) => {
            ...
            updateSecurityEvent({
                ...
                if (res1.code === 200) {
                    ...
                    viewSecurityEvent({ id, sourceType }).then((res) => {
                        if (res.code === 200) {
                            setDetail(res.data);
    ...
    return (
        ...
        >
                    ...
                    <Form.Item label="情报类型" labelCol={{ span: 2 }} wrapperCol={{ span: 22 }}>
                        <Radio.Group
                            ...
                            onChange={(e) => {
                                if (e.target.value === 1) {
                                    history.push(
                                        `/intelligence/approved-${type}th/0day${history.location.search}`,
                                    );
                                }
                            }}
                        >
            ...
            <ScCardDetail title="恶意样本分析结果">
                <Analyse source="verifyeven" detail={detail} />
            </ScCardDetail>
            <ScCardDetail
                title="审批进度"
                extra={
            ...
            >
                {cardStatus.card3 && <AssessInfo source="verifyeven" detail={threatProcess} />}
            </ScCardDetail>
            <Modal
                title={null}
...
export default compose(withConnect, memo)(VerifyEvenPage);
