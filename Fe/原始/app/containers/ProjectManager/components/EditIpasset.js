...
import { getIpAssetEdit, getIpAssetDetail } from '../page/api';

const { Option } = Select;
const { TextArea } = Input;

const EditIpasset = (props) => {
    const [form] = Form.useForm();
    const { id } = searchParams();
    const { iptype, handleCancelIp, idIp } = props;
    ...
    useEffect(() => {
        getIpAssetDetail({ id: idIp }).then((res) => {
            if (res.code === 200) {
                setIpData(res.data);
            } else {
                console.log('获取数据失败');
    ...
    useEffect(() => {
        form.setFieldsValue(IpData);
    }, [IpData]);
    IpData.assetType = IpData.assetTypeKey === undefined ? '' : IpData.assetTypeKey.split(',');
    return (
        <>
            <Form
                name="basic"
...
export default EditIpasset;
EditIpasset.propTypes = {
    iptype: PropTypes.array,
    handleCancelIp: PropTypes.func,
    idIp: PropTypes.number,
};
