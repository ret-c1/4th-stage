...
import { getAddSecurityWay, getCheckSecurityPolicy } from '../api';

// const { Option } = Select;
const { TextArea } = Input;

const EditSecurityPolicy = (props) => {
    const [form] = Form.useForm();
    const { id } = searchParams();
    const { handleCancel, idPolicy } = props;
    ...
    useEffect(() => {
        getCheckSecurityPolicy(idPolicy).then((res) => {
            if (res.code === 200) {
    ...
    recordData.upgradeDate = moment(Date(recordData.upgradeDate));
    recordData.recordDate = moment(Date(recordData.recordDate));
    useEffect(() => {
    ...
    return (
        <>
            ...
            >
                ...
                <Row>
                    <Col span={12}>
                        <Form.Item label="申请单编号" name="prodName">
                ...
                <Row>
                    ...
                    <Col span={12}>
                        <Form.Item label="完成日期" name="upgradeDate">
...
export default EditSecurityPolicy;
EditSecurityPolicy.propTypes = {
    idPolicy: PropTypes.number,
