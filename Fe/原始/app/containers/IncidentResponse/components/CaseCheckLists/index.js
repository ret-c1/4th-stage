...
const CaseCheckLists = (props) => {
    ...
    const checkListsChildSort = (fatherIndex, newList) => {
        newList[fatherIndex].caseChecklistDetails.forEach((item, index) => {
            const newObj = { ...item };
            newObj.sort = index;
            return newObj;
        });
    ...
    const handleChangeChild = (fatherIndex, childIndex, type, newEle) => {
        const newList = checkListsData.checklistDTOS;
        if (type === 'edit') {
            newList[fatherIndex].caseChecklistDetails.splice(childIndex, 1, newEle);
        }
        if (type === 'delete') {
            newList[fatherIndex].caseChecklistDetails.splice(childIndex, 1);
            checkListsChildSort(fatherIndex, newList);
        }
        if (type === 'add') {
            if (!newList[fatherIndex].caseChecklistDetails) {
                newList[fatherIndex].caseChecklistDetails = [];
            }
            newList[fatherIndex].caseChecklistDetails.push(newEle);
        }
        setCheckListData({ ...checkListsData, checklistDTOS: newList });
        // onChange({ checklistDTOS: newList });
        PubMessage('success', '成功');
        ...
        <React.Fragment key="checkList">
            {checkListsData.checklistDTOS &&
                checkListsData.checklistDTOS.map((item, fatherIndex) => {
                    ...
                    return (
                        <React.Fragment
                            key={`${item.id.toString()}_${fatherIndex.toString()}_${name}`}
                        >
                            <StyledFatherWrapper
                            ...
                            {item.caseChecklistDetails &&
                                item.caseChecklistDetails.map((child, childIndex) => (
                                    <React.Fragment key={`${child.id}_${childIndex.toString()}`}>
                                        <div
                                            style={{
