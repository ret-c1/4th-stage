...
const ReportPage = () => {
    ...
    useEffect(() => {
        ...
        getEmergencyInfo(id).then((res) => {
            if (res.code === 200) {
                reportInfo.相关说明 = `本文档是由“”于${moment(res.realDiscoverTime).format(
                    'YYYY-MM-DD',
                )}针对${res.clientName || ''}${res.intranetSystem ||
                    ''}进行应急响应工作所提交的报告资料`;
                reportInfo.事件概述 = `${moment(res.realDiscoverTime).format(
                    'YYYY-MM-DD',
                )},“”接到${res.clientName || ''}相关工作人员电话，告知${res.introduction ||
                    ''}，请求对该系统进行应急响应，项目经理${res.emergencyManager ||
                    ''}派安全工程师${
                    ...
                    let timer = setInterval(() => {
                        ...
                        generatorStatus(statusParam).then((statusRes) => {
                            if (statusRes.code === 200) {
                                ...
                                PubMessage('error', statusRes.message);
                                clearInterval(timer);
                                timer = null;
                                setIsShow(false);
                            }
